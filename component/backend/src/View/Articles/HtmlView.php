...
namespace Akeeba\Component\DocImport\Administrator\View\Articles;

defined('_JEXEC') || die;

use Akeeba\Component\DocImport\Administrator\Model\ArticlesModel;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		ToolbarHelper::title(Text::_('COM_DOCIMPORT_TITLE_ARTICLES'), 'docimport');
		...
		if ($canCreate)
		{
			ToolbarHelper::addNew('articles.add');
		}
		...
		if ($canDelete || $canEditState)
		{
			...
			if ($canEditState)
			{
				$childBar->publish('articles.publish')
					->icon('fa fa-check-circle')
					->text('JTOOLBAR_PUBLISH')
					->listCheck(true);

				$childBar->unpublish('articles.unpublish')
		...
		ToolbarHelper::link(
			Route::_('index.php?option=com_docimport&view=categories'),
			'COM_DOCIMPORT_TITLE_CATEGORIES',
			'folder'
		);
	}
}